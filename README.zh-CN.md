...
本地优先的电子书整理、元数据分析、预览和书库维护工具，支持 macOS、Windows 与 Linux。
...
- 通过用户指定目录同步数据库和跨平台偏好；API Key 与 OAuth 凭据默认仅保存在本机。
...
默认数据目录是 `~/.book_organizer/`。其中包括配置、数据库及用户自行配置的 Google Drive OAuth 文件。跨平台偏好可以同步，但 API Key、Token 和 client secret 默认不参与同步。预览不会记录阅读进度，不修改图书源文件。
...
Calibre 的 `ebook-convert` 只用于可选的 PDF 转换，不随程序捆绑。
